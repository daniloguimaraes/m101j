...
public class JavaClassFinder  {
	...
	public <T> List<Class<? extends T>> findAllMatchingTypes(Class<T> toFind) {
		return findAllMatchingTypes(toFind, false);
	}
	
	@SuppressWarnings("unchecked")
	public <T> List<Class<? extends T>> findAllMatchingTypes(Class<T> toFind, boolean removeClassToFind) {
		...
		if (removeClassToFind) {
			returnedClasses.remove(toFind);
		}
